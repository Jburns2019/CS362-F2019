...
int baron(struct gameState *state, int choice1, int currentPlayer) {
    ...
    if (choice2 > 0) { //Boolean true or going to discard an estate //bug 1
        ...
        while(card_not_discarded != 2) {//bug 2
...
int tribute(struct gameState *state, int nextPlayer, int tributeRevealedCards[]) {
...
int ambassador(int choice1, int choice2, int handPos, struct gameState *state, int currentPlayer) {
...
int mine(int i, int j, int currentPlayer, int choice1, int choice 2, struct gameState *state, int handPos) {
    j = state->hand[currentPlayer][choice1];  //store card we will trash
    if (state->hand[currentPlayer][choice1] < copper + 1 || state->hand[currentPlayer][choice1] > gold || choice2 > treasure_map || choice2 < curse || (getCost(state->hand[currentPlayer][choice1]) + 3) > getCost(choice2) ) {//bug 1
        return -1;
    }
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
    ...
    case mine:
        return mine(i, j, currentPlayer, choice1, choice 2, state, handPos);
    ...
    case tribute:
        return tribute(state, nextPlayer, tributeRevealedCards);

    case ambassador:
        return ambassador(choice1, choice2, handPos, state, currentPlayer);
